...
func (f *FormatRequest) Call() (interface{}, string) {
	...
	af, sourceAdj, identAdj, err := f.Parse(fset, f.Filename, src, true)
	if err != nil {
	...
	res, err := f.Format(fset, af, sourceAdj, identAdj, src, conf)
	if err != nil {
		return nil, err.Error()
	}

	return FormatResponse{Src: string(res)}, ""
}

// Parse parses src, which was read from the named file,
// as a Go source file, declaration, or statement list.
func (f *FormatRequest) Parse(fset *token.FileSet, filename string, src []byte, fragmentOk bool) (
	file *ast.File,
...
func (f *FormatRequest) isSpace(b byte) bool {
	return b == ' ' || b == '\t' || b == '\n' || b == '\r'
}
